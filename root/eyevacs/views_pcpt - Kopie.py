...
#     pub.save()
#
def createBaselineTasklist(condition, ct_size):
    # global ctlist
    # global ctlistIDs
    cts = []
...
def setChoiceTasks(pub):
    ...
    if group == 'baselinelow':
        ctlist = createBaselineTasklist('low', pub.ct_size)
    if group == 'baselinehigh':
        ctlist = createBaselineTasklist('high', pub.ct_size)
    if group == 'increasing':
        ctlist = createTasklist('increasing', pub.ct_size)
        ctlist = orderTasklist(ctlist, 'increasing')
    if group == 'decreasing':
        ctlist = createTasklist('decreasing', pub.ct_size)
        ctlist = orderTasklist(ctlist, 'decreasing')
    return ctlist
...
# def getTask(task_nr):
def makeTaskDict(ctask, taskcounter):
