...
def singlePageDebug(request, exp_id, pcpt_id):
    ...
    data['pub'] = pub
    data['pages'] = allUrls(pcpt_id, exp_id)
...
def decisionsequence(request, exp_id, pcpt_id, ct_page):
    ...
    this_task = ctlist[pub.taskcounter]
    ctdict = pcpt.makeTaskDict(this_task)
    data.update(ctdict)
...
def loadingsequence(request, exp_id, pcpt_id, ct_page):
    ...
    try:
        Thread(target= waitEcho).start() #dont need args if unused
